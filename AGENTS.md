...
3. Run `python scripts/release/make_release_zip.py` — it zips the full source tree (excluding build outputs) plus the release binaries at the zip root, and regenerates `SHA256SUMS.txt` for the GitHub Release assets. Each output is written as `<name>.part`; the checksum file is renamed into place first and the zip last, and a failed zip rename rewrites the previous checksum file, so a failed run leaves the previous consistent (zip, checksums) pair rather than a truncated or mismatched one (a read-only target, a directory squatting on the target name, or a read-only leftover `.part` is rejected by name before anything is built, and an empty source set aborts instead of shipping binaries only; the only residual windows are a process kill between the two renames and an exclusive lock taken by something else at rename time). To verify the checksums locally, drop the binary under its release name (`KimiPlanbarTray-rust.exe`) at the repo root and run `sha256sum -c SHA256SUMS.txt`; that name is gitignored and excluded by the packaging script, so it cannot end up inside the zip (`__pycache__` / `*.pyc` leftovers are excluded the same way).
