...
3. `python scripts/release/make_release_zip.py` packages a zip of the source snapshot + binaries and generates `SHA256SUMS.txt`: each is written as `<name>.part`, the checksum file is renamed into place first and the zip last, and a failed zip rename rewrites the previous checksum file, so a failed run always leaves the previous consistent pair (a read-only target, a directory squatting on the target name, or a read-only leftover `.part` is rejected by name before anything is built; an empty source set aborts instead of shipping binaries only; the only residual windows are a process kill between the two renames and an exclusive lock taken at rename time)
